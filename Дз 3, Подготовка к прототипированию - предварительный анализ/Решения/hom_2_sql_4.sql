--4) Определение кредитового оборота по счету в заданном периоде дат, в рублях.
...
--ct_turnover	NUMERIC	Сумма кредитового оборота для счета за выбранный период дат.*
...
SELECT
	...
	SUM(
        CASE
            WHEN ac.Kind_Account = 'А' AND ar.Real_Payer = ac.account THEN ar.Sum
            WHEN ac.Kind_Account = 'П' AND ar.Real_Receiver = ac.account THEN ar.Sum
            ELSE 0
        END
    ) AS ct_turnover
FROM
